    ...
    class MeshDataStructure {
        ...
        setHalfEdge(a: number,b: number) {
            ...
            if((this.HalfEdgeDict[halfEdgeKey] as _HALFEDGE_)) {
                ...
                (this.HalfEdgeDict[halfEdgeKey] as _HALFEDGE_).twin = halfEdgeKey;
                this.edge_no--;
            }
        ...
        addFace(face: number[]) {
            ...
            if(!this.faces.includes(this.face_vertices_tmp)) {
                ...
                for(const i in face) {
                    ...
                    if((this.HalfEdgeDict[halfEdgeKey] as _HALFEDGE_) !== undefined) {
                        (this.HalfEdgeDict[halfEdgeKey] as _HALFEDGE_).next = halfEdgeKey;
                    }
        ...
        removeFace(face : number[]){
            ...
            if(this.faces.includes(this.face_vertices_tmp)) {
                ...
                for(const edge in this.HalfEdgeDict){
                    ...
                    if((this.HalfEdgeDict[edge] as _HALFEDGE_).face_vertices === this.face_vertices_tmp)
                    {
                        found_edges++;
                        let cur_halfEdge = (this.HalfEdgeDict[edge] as _HALFEDGE_).next; // update the halfedge
                        let twinHalfEdgeKey = (this.HalfEdgeDict[edge] as _HALFEDGE_).twin;
                        if(! (this.HalfEdgeDict[edge] as _HALFEDGE_)) this.edge_no--; // If the twin does not exist decrease edge number
                        delete this.HalfEdgeDict[edge]; // delete the previous halfedge

                        while (found_edges < face.length)
                        {
                            cur_halfEdge = (this.HalfEdgeDict[edge] as _HALFEDGE_).next; // update the halfedge
                            twinHalfEdgeKey = (this.HalfEdgeDict[edge] as _HALFEDGE_).twin;
                            if(!(this.HalfEdgeDict[edge] as _HALFEDGE_)) this.edge_no--; // If the twin does not exist decrease edge number   
                            delete this.HalfEdgeDict[edge]; // delete the previous halfedge
                            found_edges++;
                        }
        ...
        getFacesofEdge(edge : string){
            return [(this.HalfEdgeDict[edge] as _HALFEDGE_).face_vertices, (this.HalfEdgeDict[(this.HalfEdgeDict[edge] as _HALFEDGE_).twin] as _HALFEDGE_).face_vertices];
        }
