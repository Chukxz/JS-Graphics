(function () {
    ...
    abstract class BinarySearch<T>{
        ...
        recursive(elem: T,arr: T[],min: number,max: number) // min = 0, max = inputArray.length - 1
        {
            if(min > max) return -1;
                ...
                else if(this.satisfies() === -1) this.recursive(elem,arr,min,mid - 1);
                else this.recursive(elem,arr,mid + 1,max);
            }
        }
    ...
    const shape = new CreateTorus();
    shape.calculatePoints();

    console.log(shape instanceof CreateObject)
