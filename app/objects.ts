...
class ObjectRendering {
    ...
    private deleteObjectHelper(instance_number_input: number,index: number) {
        ...
        if(instance_number_input in this.selected_object_instances) this.selected_object_instances.delete(instance_number_input);
    }
    ...
    select_all_objects() { for(const index in this.objects) { this.select_object(Number(index)) }; }
    ...
    renderObject() {
        ...
        const renderedObjectVertices: _4D_VEC_[] = [];
        ...
        for(const index in object.rendered_points_list) {
            ...
            if(typeof rendered_vertex === "undefined") return undefined;
