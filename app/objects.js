...
class ObjectRendering extends Miscellanous {
    ...
    deleteObjectHelper(instance_number_input) {
        ...
        if (instance_number_input === this.current_object_instance)
            this.current_object_instance = Number(Object.keys(this.objects_dict.object_dict)[0]);
    }
    addObjects(object) {
    ...
    select_all_objects() { for (const index in this.objects_dict.object_dict) {
        this.select_object(Number(this.objects_list[index].instance));
    } ; }
    ...
    translateObject(translation_array, incremental = false) {
        ...
        if (object.object_state === "local")
            return;
        if (incremental === true)
            translation_array = _Matrix.matAdd(object.object_translation_array, translation_array);
        object.rendered_points_list = [];
        for (const index in object.points_list) {
            const orig_pt = object.points_list[index];
            const original_vector = [orig_pt.x, orig_pt.y, orig_pt.z];
    ...
    scaleObject(scaling_array, incremental = false) {
        ...
        if (typeof object === "undefined")
            return;
        if (incremental === true)
            scaling_array = _Matrix.matAdd(object.object_scaling_array, scaling_array);
        object.rendered_points_list = [];
        for (const index in object.points_list) {
            const orig_pt = object.points_list[index];
            const original_vector = [orig_pt.x, orig_pt.y, orig_pt.z];
