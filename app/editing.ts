...
class CreateCylinder_SVG_Indicator extends Shape_SVG_Indicator {
    ...
    start_event(){
        for(const ellipse of this.ellipses) ellipse.event_in();
        for(const line of this.lines) line.event_in();
        this.tooltip_class.call_function_in();
        this.hovered = true;
    }
    ...
    end_event(){
        for (const line of this.lines) line.event_out();
        for(const ellipse of this.ellipses) ellipse.event_out();
        this.tooltip_class.call_function_out();
        this.hovered = false;
    }
}
